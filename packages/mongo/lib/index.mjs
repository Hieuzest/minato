...
import { Driver, executeUpdate, Field as Field2, hasSubquery, RuntimeError, z } from "minato";
import { URLSearchParams } from "url";

// src/builder.ts
import { isNullable, mapValues } from "cosmokit";
import { Eval, Field, flatten, isAggrExpr, isComparable, isEvalExpr, isFlat, makeRegExp, Selection, Type, unravel } from "minato";
...
var Builder = class _Builder {
  ...
  load(value, type) {
    ...
    if (Type.isType(type) || isEvalExpr(type)) {
      ...
      const types = this.driver.database[symbols.original]?.types ?? this.driver.database.types;
      const ancestor = types[type.type]?.type;
      let res = this.load(value, ancestor ? Type.fromField(ancestor) : void 0);
      res = converter?.load ? converter.load(res) : res;
      res = Type.transform(res, type, (value2, type2) => this.load(value2, type2));
      return res;
    }
...
var MongoDriver = class extends Driver {
  static name = "mongo";
  client;
  db;
  mongo = this;
  ...
  async _createFields(table) {
    ...
    for (const key in fields) {
      ...
      if (oldKey) {
        ...
        found.fields.push(key);
        if (isNullable2(initial)) continue;
  ...
  async set(sel, update) {
    ...
    if (hasSubquery(sel.query) || Object.values(update).some((x) => hasSubquery(x))) {
      ...
      const $set = mapValues2(update, (item, key) => transformer.toUpdateExpr(item, model.getType(key)));

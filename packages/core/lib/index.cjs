...
((Type2) => {
  ...
  function fromPrimitive(value) {
    if ((0, import_cosmokit.isNullable)(value)) return fromField("expr");
    ...
    else if (import_cosmokit.Binary.is(value)) return fromField("binary");
  ...
  function fromField(field) {
    ...
    else if (typeof field === "string") return (0, import_cosmokit.defineProperty)({ type: field }, Type2.kType, true);
  ...
  function getInner(type, key) {
    ...
    if ((0, import_cosmokit.isNullable)(key)) return;
...
((Relation4) => {
  ...
  function parse(def, key, model, relmodel, subprimary) {
    ...
    const fields = def.fields ?? (subprimary || def.type === "manyToOne" || def.type === "oneToOne" && (model.name === relmodel.name || !(0, import_cosmokit2.makeArray)(relmodel.primary).every((key2) => !relmodel.fields[key2]?.nullable)) ? (0, import_cosmokit2.makeArray)(relmodel.primary).map((x) => `${key}.${x}`) : model.primary);
    const relation = {
      type: def.type,
...
var Model = class {
  ...
  extend(fields = {}, config = {}) {
    ...
    indexes.map((x) => this.parseIndex(x)).forEach((index) => this.indexes.some((ind) => (0, import_cosmokit2.deepEqual)(ind, index)) || this.indexes.push(index));
  ...
  parseIndex(index) {
    if (typeof index === "string" || Array.isArray(index)) {
      return {
        name: `index:${this.name}:` + (0, import_cosmokit2.makeArray)(index).join("+"),
        unique: false,
        keys: Object.fromEntries((0, import_cosmokit2.makeArray)(index).map((key) => [key, "asc"]))
      };
    } else {
      return {
  ...
  checkIndex(index) {
    for (const key of typeof index === "string" || Array.isArray(index) ? (0, import_cosmokit2.makeArray)(index) : Object.keys(index.keys)) {
      if (!this.fields[key]) {
  ...
  resolveValue(field, value) {
    if ((0, import_cosmokit2.isNullable)(value)) return value;
  ...
  resolveModel(obj, model) {
    ...
    if ((0, import_cosmokit2.isNullable)(obj) || !model.inner) return obj;
  ...
  getType(key) {
    if (!this.type) (0, import_cosmokit2.defineProperty)(this, "type", Type.Object((0, import_cosmokit2.mapValues)(this.fields, (field) => Type.fromField(field))));
...
function hasSubquery(value) {
  ...
  return Object.entries(value).filter(([k]) => k.startsWith("$")).some(([k, v]) => {
    if ((0, import_cosmokit3.isNullable)(v) || isComparable(v)) return false;
...
function comparator(key, callback) {
  operators[`$${key}`] = (args, data) => {
    ...
    if ((0, import_cosmokit3.isNullable)(left) || (0, import_cosmokit3.isNullable)(right)) return true;
...
Eval3.object = (fields) => {
  if (fields.$model) {
    ...
    return Object.assign(Eval3("object", fields, Type.Object((0, import_cosmokit3.mapValues)(fields, (value) => Type.fromTerm(value)))), fields);
  }
  fields = (0, import_cosmokit3.filterKeys)(fields, (key) => key !== "$object");
  return Object.assign(Eval3("object", fields, Type.Object((0, import_cosmokit3.mapValues)(fields, (value) => Type.fromTerm(value)))), fields);
};
...
function isEmpty(value) {
  if ((0, import_cosmokit4.isNullable)(value)) return true;
...
var Executable = class {
  ...
  constructor(driver, payload) {
    ...
    (0, import_cosmokit5.defineProperty)(this, "row", createRow(this.ref, Eval3.object(createRow(this.ref, {}, "", this.model)), "", this.model));
  }
  isCallaback(query) {
    if (typeof query !== "function") return false;
    const fields = query(this.row);
    return isEvalExpr((0, import_cosmokit5.omit)(fields, ["$object"]));
  }
  resolveQuery(query = {}) {
...
var Selection = class _Selection extends Executable {
  ...
  join(name, selection, callback = () => Eval3.and(), optional = false) {
    ...
    if (optional) {
      return this.driver.database.join({ [this.ref]: this, [name]: selection }, (t) => callback(t[this.ref], t[name]), { [this.ref]: false, [name]: true }).project({ ...fields, [name]: (row) => Eval3.ignoreNull(Eval3.object((0, import_cosmokit5.mapValues)(joinFields, (x) => x(row)))) });
    } else {
      return this.driver.database.join({ [this.ref]: this, [name]: selection }, (t) => callback(t[this.ref], t[name])).project({ ...fields, [name]: (row) => Eval3.ignoreNull(Eval3.object((0, import_cosmokit5.mapValues)(joinFields, (x) => x(row)))) });
    }
  }
  _action(type, ...args) {
  ...
  evaluate(callback) {
    ...
    if (isAggrExpr(expr)) (0, import_cosmokit5.defineProperty)(expr, Type.kType, Type.Array(Type.fromTerm(expr)));
...
var Database = class _Database extends import_cordis.Service {
  ...
  extend(name, fields, config = {}) {
    ...
    if (Array.isArray(model.primary) || model.fields[model.primary].relation) {
      model.primary = (0, import_cosmokit6.deduplicate)((0, import_cosmokit6.makeArray)(model.primary).map((key) => model.fields[key].relation?.fields || key).flat());
    }
  ...
  parseField(field, transformers = [], setInitial, setField) {
    ...
    if (typeof field === "object" && field.load && field.dump) {
      if (type.inner) type = Type.fromField(this.define({ ...(0, import_cosmokit6.omit)(midfield, ["load", "dump"]), type }));
  ...
  select(table, query, include) {
    ...
    if (include) include = (0, import_cosmokit6.filterKeys)(include, (key) => !!modelFields[key]?.relation);
      ...
      for (const key in include) {
        ...
        if (relation.type === "oneToOne" || relation.type === "manyToOne") {
          sel = whereOnly ? sel : sel.join(key, this.select(
            relation.table,
            typeof include[key] === "object" ? (0, import_cosmokit6.filterKeys)(include[key], (k) => !relmodel.fields[k]?.relation) : {},
            typeof include[key] === "object" ? (0, import_cosmokit6.filterKeys)(include[key], (k) => !!relmodel.fields[k]?.relation) : include[key]
...
var Driver = class {
  ...
  async prepareIndexes(table) {
    ...
    for (const index of indexes) {
      ...
      } else if (!(0, import_cosmokit7.deepEqual)(oldIndex, index)) {
...
var queryOperators = {
  ...
  // existence
  $exists: /* @__PURE__ */ __name((query, data) => query !== (0, import_cosmokit8.isNullable)(data), "$exists"),
  // comparison

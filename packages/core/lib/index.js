...
// src/database.ts
import { deduplicate, defineProperty as defineProperty5, filterKeys as filterKeys4, isNullable as isNullable5, makeArray as makeArray2, mapValues as mapValues5, noop, omit as omit2, pick, remove } from "cosmokit";
...
// src/eval.ts
import { defineProperty as defineProperty3, filterKeys as filterKeys2, isNullable as isNullable3, mapValues as mapValues3 } from "cosmokit";
  ...
  function transform(value, type, callback) {
    if (!isNullable(value) && type?.inner) {
      if (Type2.isArray(type)) {
        const innerType = Type2.getInner(type);
        return value.map((x) => callback(x, innerType)).filter((x) => !type.ignoreNull || !isEmpty(x));
      } else {
...
var Relation;
((Relation4) => {
  const Marker = /* @__PURE__ */ Symbol("minato.relation");
...
((Field2) => {
  ...
  const NewType = /* @__PURE__ */ Symbol("minato.newType");
  const regexp = /^(\w+)(?:\((.+)\))?$/;
...
var Model = class {
  ...
  extend(fields = {}, config = {}) {
    ...
    defineProperty2(this, "type", Type.Object(mapValues2(this.fields, (field) => Type.fromField(field))));
  }
  parseIndex(index) {
...
Eval3.object = (fields) => {
  if (fields.$model) {
    ...
    return Object.assign(Eval3("object", fields, Type.Object(mapValues3(fields, (value) => Type.fromTerm(value)))), fields);
  }
  fields = filterKeys2(fields, (key) => key !== "$object");
  return Object.assign(Eval3("object", fields, Type.Object(mapValues3(fields, (value) => Type.fromTerm(value)))), fields);
};
...
// src/selection.ts
import { defineProperty as defineProperty4, filterKeys as filterKeys3, mapValues as mapValues4, omit } from "cosmokit";
...
var Executable = class {
  ...
  constructor(driver, payload) {
    ...
    defineProperty4(this, "row", createRow(this.ref, Eval3.object(createRow(this.ref, {}, "", this.model)), "", this.model));
  }
  isCallaback(query) {
...
var Selection = class _Selection extends Executable {
  ...
  join(name, selection, callback = () => Eval3.and(), optional = false) {
    ...
    if (optional) {
      return this.driver.ctx.model.join({ [this.ref]: this, [name]: selection }, (t) => callback(t[this.ref], t[name]), { [this.ref]: false, [name]: true }).project({ ...fields, [name]: (row) => Eval3.ignoreNull(Eval3.object(mapValues4(joinFields, (x) => x(row)))) });
    } else {
      return this.driver.ctx.model.join({ [this.ref]: this, [name]: selection }, (t) => callback(t[this.ref], t[name])).project({ ...fields, [name]: (row) => Eval3.ignoreNull(Eval3.object(mapValues4(joinFields, (x) => x(row)))) });
    }
  }
  _action(type, ...args) {
...
var Database = class _Database extends Service {
  ...
  static transact = /* @__PURE__ */ Symbol.for("minato.transact");
  static migrate = /* @__PURE__ */ Symbol.for("minato.migrate");
  ...
  parseField(field, transformers = [], setInitial, setField) {
    ...
    if (typeof field === "object" && field.load && field.dump) {
      if (type.inner) type = Type.fromField(this.define({ ...omit2(midfield, ["load", "dump"]), type }));
  ...
  select(table, query, include) {
    ...
    if (include) include = filterKeys4(include, (key) => !!modelFields[key]?.relation);
    ...
    sel.query = omit2(sel.query, Object.keys(include ?? {}));
    if (Object.keys(sel.query.$not ?? {}).length) {
      sel.query.$not = omit2(sel.query.$not, Object.keys(include ?? {}));
      ...
      for (const key in include) {
        ...
        if (relation.type === "oneToOne" || relation.type === "manyToOne") {
          sel = whereOnly ? sel : sel.join(key, this.select(
            relation.table,
            typeof include[key] === "object" ? filterKeys4(include[key], (k) => !relModel.fields[k]?.relation) : {},
            typeof include[key] === "object" ? filterKeys4(include[key], (k) => !!relModel.fields[k]?.relation) : include[key]
  ...
  async set(table, query, update) {
    ...
    if (relations.length) {
      return await this.ensureTransaction(async (database) => {
        ...
        let baseUpdate = omit2(rawUpdate(sel.row), relations.map(([key]) => key));
  ...
  makeProxy(marker, getDriver) {
    ...
    const database = new Proxy(this, {
      get: /* @__PURE__ */ __name((target, p, receiver) => {
        if (p === /* @__PURE__ */ Symbol.for("cordis.tracker")) return void 0;
  ...
  async createOrUpdate(table, data, upsert = true) {
    ...
    for (const key of [...tasks]) {
      if (!key) {
        ...
        if (upsert) {
          await sel._action("upsert", [sel.model.format(omit2(data, tasks))], keys).execute();
        } else {
          Object.assign(data, await sel._action("create", sel.model.create(omit2(data, tasks))).execute());
        }
        continue;
      }
...
var _Driver_decorators, _init;
_Driver_decorators = [Inject("model")];
var Driver = class {
  ...
  async *[Service2.init]() {
    ...
    yield () => remove2(this.ctx.model.drivers, this);
    this.ctx.model.refresh();
    const database = this.database = Object.create(this.ctx.model);
    defineProperty6(database, "ctx", this.ctx);
    defineProperty6(database, Service2.tracker, {
      associate: "database",
      property: "ctx"
    });
    defineProperty6(database, "_driver", this);
    yield this.ctx.provide("database", database);
  }
  model(table) {
    if (typeof table === "string") {
      const model2 = this.ctx.model.tables[table];
...
Driver = __decorateElement(_init, 0, "Driver", _Driver_decorators, Driver);
__runInitializers(_init, 1, Driver);

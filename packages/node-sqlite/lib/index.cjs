...
var SQLiteBuilder = class extends import_sql_utils.Builder {
  constructor(driver, tables) {
    ...
    this.evalOperators.$log = ([left, right]) => (0, import_cosmokit.isNullable)(right) ? `ln(${this.parseEval(left)})` : `ln(${this.parseEval(left)}) / ln(${this.parseEval(right)})`;
...
var SQLiteDriver = class extends import_minato2.Driver {
  ...
  async start() {
    ...
    const DatabaseSync = await import("node:sqlite").then((m) => m.DatabaseSync).catch((e) => {

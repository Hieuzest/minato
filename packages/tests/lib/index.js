...
var shape_default = /* @__PURE__ */ __name((({ Assertion }) => {
  function checkShape(expect12, actual, path, ordered) {
    if (actual === expect12 || Number.isNaN(expect12) && Number.isNaN(actual)) return;
    function formatError(expect13, actual2) {
      return `expected to have ${expect13} but got ${actual2} at path ${path}`;
    }
    __name(formatError, "formatError");
    if (isNullable(expect12) && isNullable(actual)) return;
    if (!expect12 || ["string", "number", "boolean", "bigint"].includes(typeof expect12)) {
      return formatError(inspect(expect12), inspect(actual));
    }
    if (expect12 instanceof Date) {
      if (!(actual instanceof Date) || +expect12 !== +actual) {
        return formatError(inspect(expect12), inspect(actual));
      }
      return;
    }
    if (Binary.is(expect12)) {
      if (!Binary.is(actual) || !deepEqual(actual, expect12)) {
        return formatError(inspect(expect12), inspect(actual));
      }
      return;
    }
    if (actual === null) {
      const type2 = Object.prototype.toString.call(expect12).slice(8, -1).toLowerCase();
      return formatError(`a ${type2}`, "null");
    }
    if (!ordered && Array.isArray(expect12) && Array.isArray(actual)) {
      if (!isSubsetOf(expect12, actual, (x, y) => !checkShape(x, y, `${path}/`, ordered), false, false)) {
        return `expected same shape of members`;
      }
      return;
    }
    for (const prop in expect12) {
      if (isNullable(actual[prop]) && !isNullable(expect12[prop])) {
        return `expected "${prop}" field to be defined at path ${path}`;
      }
      const message = checkShape(expect12[prop], actual[prop], `${path}${prop}/`, ordered);
      if (message) return message;
    }
  }
  __name(checkShape, "checkShape");
  Assertion.addMethod("shape", function(expect12) {
    var ordered = flag(this, "ordered");
    const message = checkShape(expect12, this._obj, "/", ordered);
    if (message) this.assert(false, message, "", expect12, this._obj);
  });
}), "default");

// src/index.ts
import { Database as Database10 } from "minato";

// src/model.ts
import { mapValues, isNullable as isNullable2, deduplicate, omit } from "cosmokit";
...
function ModelOperations(database) {
  before(() => {
    ...
    database.define("custom", {
      type: "string",
      dump: /* @__PURE__ */ __name((value) => isNullable2(value) ? value : `${value.a}|${value.b}`, "dump"),
      load: /* @__PURE__ */ __name((value) => isNullable2(value) ? value : { a: value.split("|")[0], b: +value.split("|")[1] }, "load")
    });
    const bnum = database.define({
      type: "binary",
      dump: /* @__PURE__ */ __name((value) => isNullable2(value) ? value : toBinary(String(value)), "dump"),
      load: /* @__PURE__ */ __name((value) => isNullable2(value) ? value : +fromBinary(value), "load"),
      initial: 0
    });
    const bstr = database.define({
      type: "custom",
      dump: /* @__PURE__ */ __name((value) => isNullable2(value) ? value : { a: value, b: 1 }, "dump"),
      load: /* @__PURE__ */ __name((value) => isNullable2(value) ? value : value.a, "load"),
      initial: "pooo"
    });
    database.define("string2", {
      type: "string",
      dump: /* @__PURE__ */ __name((value) => isNullable2(value) ? value : `AAA${value}`, "dump"),
      load: /* @__PURE__ */ __name((value) => isNullable2(value) ? value : value.slice(3), "load"),
      initial: ""
    });
    ...
    const baseFields = {
      ...
      bnum2: {
        type: "binary",
        dump: /* @__PURE__ */ __name((value) => isNullable2(value) ? value : toBinary(String(value)), "dump"),
        load: /* @__PURE__ */ __name((value) => isNullable2(value) ? value : +Buffer.from(value), "load"),
        initial: 0
      },
      text2: "string2"
    };
...
import { deepEqual as deepEqual2, omit as omit2 } from "cosmokit";
...
((OrmOperations2) => {
  ...
  OrmOperations2.index = /* @__PURE__ */ __name(function Index(database) {
    it("basic support", async () => {
      ...
      let added = indexes.find((ind) => deepEqual2(omit2(ind, ["name"]), index2));
      expect3(added).to.not.be.undefined;
      await driver.dropIndex("temp2", added.name);
      indexes = await driver.getIndexes("temp2");
      added = indexes.find((ind) => deepEqual2(omit2(ind, ["name"]), index2));
    ...
    it("named", async () => {
      ...
      await driver.createIndex("temp2", index2);
      let indexes = await driver.getIndexes("temp2");
      let added = indexes.find((ind) => deepEqual2(ind, index2));
      expect3(added).to.not.be.undefined;
      await driver.dropIndex("temp2", added.name);
      indexes = await driver.getIndexes("temp2");
      added = indexes.find((ind) => deepEqual2(ind, index2));
      expect3(added).to.be.undefined;
    });
    it("extend", async () => {
      const driver = Object.values(database.drivers)[0];
      ...
      const existed = indexes.find((ind) => deepEqual2(omit2(ind, ["name"]), index2));
...
import { deepEqual as deepEqual3, noop, omit as omit3 } from "cosmokit";
function MigrationTests(database) {
  ...
  it("indexes", async () => {
    ...
    expect5(indexes.find((ind) => deepEqual3(omit3(ind, ["name"]), {
      unique: false,
      keys: {
        number: "asc"
      }
    ...
    indexes = await driver.getIndexes("qux");
    expect5(indexes.find((ind) => deepEqual3(ind, {
      name: "named-index",
      unique: false,
      keys: {
        id: "asc",
...
((JsonTests2) => {
  ...
  function query(database, options = {}) {
    const { nullableComparator = true } = options;
    it("$size", async () => {
      await expect7(database.get("json.baz", {
        nums: { $size: 3 }
      })).to.eventually.deep.equal([
        { id: 1, nums: [4, 5, 6] },
        { id: 2, nums: [5, 6, 7] }
      ]);
      await expect7(database.select("json.baz", {
        nums: { $size: 3 }
      }).project({
    ...
    it("$el", async () => {
      ...
      await expect7(database.get("json.bar", {
        l: { $el: "4" }
      })).to.eventually.have.shape([
        { uid: 1, pid: 1, value: 1 }
      ]);
    ...
    it("$.in", async () => {
      await expect7(database.get("json.baz", (row) => $6.in($6.add(3, row.id), row.nums))).to.eventually.deep.equal([
        { id: 1, nums: [4, 5, 6] },
        { id: 2, nums: [5, 6, 7] }
      ]);
      await expect7(database.get("json.bar", (row) => $6.in("4", row.l))).to.eventually.have.shape([
        { uid: 1, pid: 1, value: 1 }
      ]);
      await expect7(database.get("json.bar", (row) => $6.in("b", row.la))).to.eventually.have.shape([
        { uid: 1, pid: 1, value: 0 },
        { uid: 1, pid: 1, value: 1 }
      ]);
    });
    it("$.nin", async () => {
      await expect7(database.get("json.baz", (row) => $6.nin($6.add(3, row.id), row.nums))).to.eventually.deep.equal([
        { id: 3, nums: [7, 8] }
      ]);
      await expect7(database.get("json.bar", (row) => $6.nin("4", row.l))).to.eventually.have.shape([
        { uid: 1, pid: 1, value: 0 },
        { uid: 1, pid: 2, value: 0 }
      ]);
      await expect7(database.get("json.bar", (row) => $6.nin("b", row.la))).to.eventually.have.shape([
        { uid: 1, pid: 2, value: 0 }
      ]);
    });
    it("execute nested selection", async () => {
      await expect7(database.eval("json.bar", (row) => $6.max($6.add(1, row.value)))).to.eventually.deep.equal(2);
      await expect7(database.eval("json.bar", (row) => $6.max($6.add(1, row.obj.x)))).to.eventually.deep.equal(4);
    });
    it("$get array", async () => {
      await expect7(database.get("json.baz", (row) => $6.eq($6.get(row.nums, 0), 4))).to.eventually.deep.equal([
        { id: 1, nums: [4, 5, 6] }
      ]);
      await expect7(database.get("json.baz", (row) => $6.eq(row.nums[0], 4))).to.eventually.deep.equal([
        { id: 1, nums: [4, 5, 6] }
      ]);
    });
    nullableComparator && it("$get array with expressions", async () => {
      await expect7(database.get("json.baz", (row) => $6.eq($6.get(row.nums, $6.add(row.id, -1)), 4))).to.eventually.deep.equal([
        { id: 1, nums: [4, 5, 6] }
      ]);
    });
    it("$get object", async () => {
      await expect7(database.get("json.bar", (row) => $6.eq(row.obj.o.a, 2))).to.eventually.have.shape([
        { value: 1 }
      ]);
      await expect7(database.get("json.bar", (row) => $6.eq($6.get(row.obj.o, "a"), 2))).to.eventually.have.shape([
        { value: 1 }
      ]);
    });
  }
  ...
  function modify(database) {
    it("$.object", async () => {
      ...
      await expect7(database.get("json.bax", 1)).to.eventually.deep.equal([
  ...
  function selection(database) {
    it("$.object", async () => {
      const res = await database.select("json.foo").project({
    ...
    it("$.object in json", async () => {
      const res = await database.select("json.bar").project({
        obj: /* @__PURE__ */ __name((row) => $6.object({
          num: row.obj.x,
          str: row.obj.y,
          str2: row.obj.z,
    ...
    it("$.object on cell", async () => {
      const res = await database.join(["json.foo", "json.bar"], (foo, bar) => $6.eq(foo.id, bar.pid)).groupBy("json.bar", {
        x: /* @__PURE__ */ __name((row) => $6.array($6.object(row.json.foo)), "x")
      }).execute(["x"]);
    ...
    it("$.array groupBy", async () => {
      await expect7(
        database.join(["json.foo", "json.bar"], (foo, bar) => $6.eq(foo.id, bar.pid)).groupBy(["json.foo"], {
          x: /* @__PURE__ */ __name((row) => $6.array(row.json.bar.obj.x), "x"),
          y: /* @__PURE__ */ __name((row) => $6.array(row.json.bar.obj.y), "y")
        }).orderBy((row) => row.json.foo.id).execute()
      ).to.eventually.have.shape([
      ...
      await expect7(
        database.join(["json.foo", "json.bar"], (foo, bar) => $6.eq(foo.id, bar.pid)).groupBy(["json.foo"], {
          x: /* @__PURE__ */ __name((row) => $6.array(row.json.bar.obj.x), "x"),
          y: /* @__PURE__ */ __name((row) => $6.array(row.json.bar.obj.y), "y")
        }).orderBy((row) => row.json.foo.id).execute((row) => $6.array(row.y))
      ).to.eventually.have.shape([
        ["a", "b"],
        ["c"]
      ]);
      await expect7(
        database.join(["json.foo", "json.bar"], (foo, bar) => $6.eq(foo.id, bar.pid)).groupBy(["json.foo"], {
          x: /* @__PURE__ */ __name((row) => $6.array(row.json.bar.obj.x), "x"),
          y: /* @__PURE__ */ __name((row) => $6.array(row.json.bar.obj.y), "y")
        }).orderBy((row) => row.json.foo.id).execute((row) => $6.count(row.y))
      ).to.eventually.deep.equal(2);
    });
    it("$.array groupFull", async () => {
      const res = await database.select("json.bar").groupBy({}, {
    ...
    it("$.array in json", async () => {
      const res = await database.join(["json.foo", "json.bar"], (foo, bar) => $6.eq(foo.id, bar.pid)).groupBy("json.foo", {
        ...
        x: /* @__PURE__ */ __name((row) => $6.array(row.json.bar.obj.x), "x"),
        y: /* @__PURE__ */ __name((row) => $6.array(row.json.bar.obj.y), "y"),
        z: /* @__PURE__ */ __name((row) => $6.array(row.json.bar.obj.z), "z"),
        o: /* @__PURE__ */ __name((row) => $6.array(row.json.bar.obj.o), "o")
      }).orderBy((row) => row.json.foo.id).execute();
    ...
    it("$.array with expressions", async () => {
      const res = await database.join(["json.foo", "json.bar"], (foo, bar) => $6.eq(foo.id, bar.pid)).groupBy("json.foo", {
        bars: /* @__PURE__ */ __name((row) => $6.array($6.object({
          value: row.json.bar.value,
          value2: $6.add(row.json.bar.value, row.json.foo.value)
        })), "bars"),
        x: /* @__PURE__ */ __name((row) => $6.array($6.add(1, row.json.bar.obj.x)), "x"),
        y: /* @__PURE__ */ __name((row) => $6.array(row.json.bar.obj.y), "y")
      }).orderBy((row) => row.json.foo.id).execute();
      expect7(res).to.have.shape([
        {
    ...
    it("$.array nested", async () => {
      const res = await database.join(["json.foo", "json.bar"], (foo, bar) => $6.eq(foo.id, bar.pid)).orderBy((row) => row.json.foo.id).groupBy("json.foo", {
        y: /* @__PURE__ */ __name((row) => $6.array(row.json.bar.obj.x), "y")
      }).groupBy({}, {
...
// src/performance.ts
import { $ as $9 } from "minato";
import { expect as expect10 } from "chai";
function PerformanceTests(database) {
  const benchmark = /* @__PURE__ */ __name(async (label = "", repeat, task) => {
    await task(0);
  ...
  const cycleId = /* @__PURE__ */ __name((index, size) => index % size + 1, "cycleId");
  before(async () => {
    ...
    await setup(database, "perfNested", new Array(500).fill(0).map((_, i) => ({
      ...
      items: new Array(3).fill(0).map((_2, j) => ({
        value: `value-${i}-${j}`,
        index: j
      }))
    })));
  ...
  it("bulk get flat rows", async function() {
    ...
    expect10(rows).to.have.length(2e3);
    expect10(rows[1999]).to.include({ text: "hello", number: 1999 });
  });
  it("bulk get nested rows", async function() {
    const rows = await benchmark(this.test?.title, 8, () => database.get("perfNested", {}));
    expect10(rows).to.have.length(500);
    expect10(rows[0].meta.label).to.equal("label-0");
    expect10(rows[0].items).to.have.length(3);
  });
  it("bulk upsert nested rows", async function() {
    const payload = new Array(200).fill(0).map((_, i) => ({
      ...
      items: new Array(4).fill(0).map((_2, j) => ({
        value: `updated-${i}-${j}`,
        index: j
      }))
    }));
    ...
    expect10(rows).to.have.length(3);
    expect10(rows[0].items).to.have.length(4);
  });
  it("bulk upsert direct fields", async function() {
    ...
    expect10(rows).to.have.length(3);
    expect10(rows[0].text).to.match(/^direct-/);
  });
  it("bulk upsert expression fields", async function() {
    await benchmark(this.test?.title, 8, () => database.upsert("perf", (row) => {
      return new Array(400).fill(0).map((_, i) => ({
        id: i + 1,
        number: $9.multiply(row.id, 2)
      }));
    }));
    const rows = await database.get("perf", { id: { $in: [1, 200, 400] } });
    expect10(rows).to.have.length(3);
    expect10(rows[0].number).to.equal(rows[0].id * 2);
  });
  it("bulk upsert dotted fields", async function() {
    ...
    const rows = await database.get("perfNested", { id: { $in: [1, 50, 200] } });
    expect10(rows).to.have.length(3);
    expect10(rows[0].meta.label).to.match(/^dotted-/);
  });
  it("repeated get single flat row", async function() {
    ...
    expect10(row).to.have.length(1);
    expect10(row[0].text).to.be.a("string");
  });
  it("repeated get single nested row", async function() {
    ...
    expect10(row).to.have.length(1);
    expect10(row[0].meta.label).to.be.a("string");
    expect10(row[0].items).to.have.length(3);
  });
  it("repeated set single flat row", async function() {
    ...
    expect10(rows).to.have.length(1);
    expect10(rows[0]).to.include({ text: "single-set-299", number: 299 });
  });
  it("repeated set single nested row", async function() {
    ...
    expect10(rows).to.have.length(1);
    expect10(rows[0].meta).to.include({
  ...
  it("repeated upsert single flat row", async function() {
    ...
    expect10(rows).to.have.length(1);
    expect10(rows[0]).to.include({ text: "single-upsert-299", number: 598 });
  });
  it("repeated upsert single nested dotted row", async function() {
    ...
    const rows = await database.get("perfNested", { id: 1 });
    expect10(rows).to.have.length(1);
    expect10(rows[0].meta).to.include({ label: "single-dotted-199", score: 199 });
  });
  it("repeated create and remove single flat row", async function() {
    ...
    expect10(created.id).to.be.a("number");
    const rows = await database.get("perf", { text: created.text });
    expect10(rows).to.have.length(0);
  });
}
  ...
  return util.eql(expected, actual, {
    comparator: /* @__PURE__ */ __name((expected2, actual2) => {
      ...
      if (type(expected2) === "Object" && type(actual2) === "Object") {
        ...
        return [...keys].every((key) => config.deepEqual(expected2[key], actual2[key]));
      }
      return null;
    }, "comparator")
  });
...
// src/index.ts
import { expect as expect11 } from "chai";
var Keywords = ["name"];
...
function resolveGetDb(source) {
  if (typeof source === "function") return source;
  if (source instanceof Database10) return () => source;
  return () => source.model;
}
__name(resolveGetDb, "resolveGetDb");
function createLazyDatabase(getDb) {
...
__name(createLazyDatabase, "createLazyDatabase");
function createUnit(target, root = false) {
  const test = /* @__PURE__ */ __name((source, options = {}, overrideOptions) => {
    ...
    function callback() {
      ...
      for (const key in target) {
        ...
        test[key](getDb, options[key], overrideOptions?.[key] === true ? void 0 : overrideOptions?.[key]);
      }
    }
...
var index_default = createUnit(Tests, true);
export {

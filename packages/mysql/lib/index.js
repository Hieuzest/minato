...
import { Driver as Driver2, executeUpdate, Field as Field2, RuntimeError } from "minato";
import { escapeId as escapeId2, isBracketed as isBracketed2 } from "@minatojs/sql-utils";

// src/builder.ts
import { Builder, escapeId, isBracketed } from "@minatojs/sql-utils";
import { Binary, isNullable, Time } from "cosmokit";
import { Field, isAggrExpr, isEvalExpr, randomId, Type } from "minato";
var MySQLBuilder = class extends Builder {
  constructor(driver, tables, compat = {}) {
    ...
    this.transformers["boolean"] = {
      ...
      dump: /* @__PURE__ */ __name((value) => isNullable(value) ? value : value ? 1 : 0, "dump")
    };
    ...
    this.transformers["time"] = {
      ...
      dump: /* @__PURE__ */ __name((value) => isNullable(value) ? value : Time.template("yyyy-MM-dd hh:mm:ss.SSS", value), "dump")
    };
  ...
  toUpdateExpr(item, key, field, upsert) {
    ...
    for (const prop in item) {
      ...
      const v = isEvalExpr(item[prop]) ? this.transform(this.parseEval(item[prop]), item[prop], "encode") : this.transform(this.escape(item[prop], type), type, "encode");
...
function createIndex(keys) {
  return makeArray(keys).map(escapeId2).join(", ");
}
__name(createIndex, "createIndex");
var _MySQLDriver_decorators, _init, _a;
_MySQLDriver_decorators = [Inject("logger", false)];
var MySQLDriver = class extends (_a = Driver2) {
  static name = "mysql";
  pool;
  ...
  async prepare(name) {
    ...
    for (const key in fields) {
      ...
      if (key === primary && autoInc) {
        ...
        if (!isNullable2(initial) && !typedef.startsWith("text") && !typedef.endsWith("blob")) {
          def += " default " + this.sql.escape(initial, fields[key]);
        }
        if (!column && !isNullable2(initial) && (typedef.startsWith("text") || typedef.endsWith("blob"))) {
      ...
      if (!column) {
        ...
        update.push(`CHANGE ${escapeId2(column.COLUMN_NAME)} ${def}`);
      }
    }
    if (!columns.length) {
      ...
      for (const key in foreign) {
        ...
        create.push(`FOREIGN KEY (${escapeId2(key)}) REFERENCES ${escapeId2(table2)} (${escapeId2(key2)})`);
      }
    }
    for (const key of unique) {
      ...
      const name2 = "unique:" + makeArray(key).join("+");
      if (!oldIndex) {
        create.push(`UNIQUE INDEX ${escapeId2(name2)} (${createIndex(key)})`);
      } else if (shouldUpdate) {
        create.push(`UNIQUE INDEX ${escapeId2(name2)} (${createIndex(key)})`);
        update.push(`DROP INDEX ${escapeId2(oldIndex.INDEX_NAME)}`);
      }
    }
    if (!columns.length) {
      this.ctx.logger?.info("auto creating table %c", name);
      return this.query(`CREATE TABLE ${escapeId2(name)} (${create.join(", ")}) COLLATE = ${this.sql.escape(this.config.charset ?? "utf8mb4_general_ci")}`);
    }
    const operations = [
    ...
    if (operations.length) {
      ...
      await this.query(`ALTER TABLE ${escapeId2(name)} ${operations.join(", ")}`);
    }
    if (Object.keys(alterInit).length) {
      await this.query(`UPDATE ${escapeId2(name)} SET ${Object.entries(alterInit).map(([key, val]) => `${escapeId2(key)} = ${val}`).join(", ")}`);
    }
    const dropKeys = [];
    await this.migrate(name, {
      ...
      finalize: /* @__PURE__ */ __name(async () => {
        ...
        await this.query(`ALTER TABLE ${escapeId2(name)} ${dropKeys.map((key) => `DROP ${escapeId2(key)}`).join(", ")}`);
      }, "finalize")
    });
  }
...
INSERT INTO mtt VALUES(json_extract(j, concat('$[', i, ']'))); SET i=i+1; END WHILE; SELECT max(value) INTO r FROM mtt; RETURN r; END`);
  ...
  async drop(table) {
    await this.query(`DROP TABLE ${escapeId2(table)}`);
  }
  async dropAll() {
    ...
    await this.query([
      ...
      ...tables.map((table) => `DROP TABLE ${escapeId2(table)}`),
  ...
  async set(sel, data) {
    ...
    const update = updateFields.map((field) => {
      const escaped = escapeId2(field);
      return `${escaped} = ${builder.toUpdateExpr(data, field, fields[field], false)}`;
    }).join(", ");
    const sql = [...builder.prequeries, `UPDATE ${escapeId2(table)} ${ref} SET ${update} WHERE ${filter}`].join("; ");
  ...
  async remove(sel) {
    ...
    const filter = builder.parseQuery(query);
    if (filter === "0") return {};
    const result = await this.query(`DELETE FROM ${escapeId2(table)} WHERE ` + filter);
  ...
  async create(sel, data) {
    ...
    const header = await this.query([
      `INSERT INTO ${escapeId2(table)} (${keys.map(escapeId2).join(", ")})`,
  ...
  async upsert(sel, data, keys) {
    ...
    const result = await this.query([
      `INSERT INTO ${escapeId2(table)} (${initFields.map(escapeId2).join(", ")})`,
  ...
  async createIndex(table, index) {
    ...
    await this.query(
      `ALTER TABLE ${escapeId2(table)} ADD ${index.unique ? "UNIQUE" : ""} INDEX ${index.name ? escapeId2(index.name) : ""} (${keyFields})`
    );
  }
  async dropIndex(table, name) {
    await this.query(`DROP INDEX ${escapeId2(name)} ON ${escapeId2(table)}`);
  }
...
MySQLDriver = __decorateElement(_init, 0, "MySQLDriver", _MySQLDriver_decorators, MySQLDriver);
__runInitializers(_init, 1, MySQLDriver);
((MySQLDriver2) => {
  MySQLDriver2.Config = z.intersect([
    z.object({

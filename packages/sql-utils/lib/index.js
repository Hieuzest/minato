...
var Builder = class {
  ...
  compileTransform(type, method) {
    ...
    return transformer?.[method] ? (value) => transformer[method](value) : (value) => value;
  }
  compileLoad(type, root = true) {
    if (!type) return (value) => value;
    if (Type.isType(type) || isEvalExpr(type)) {
      const resolvedType = Type.isType(type) ? type : Type.fromTerm(type);
      ...
      const getInnerLoad = /* @__PURE__ */ __name((type2) => {
        ...
        if (!load) {
          load = this.compileLoad(type2, false);
      ...
      return (value) => {
        ...
        if (resolvedType.inner) {
          res = Type.transform(res, resolvedType, (x, type2) => getInnerLoad(type2 ?? Type.getInner(resolvedType))(x));
        }

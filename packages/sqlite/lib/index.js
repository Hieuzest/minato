...
import { Driver as Driver2, Eval, executeUpdate, Field as Field2, getCell, hasSubquery, isEvalExpr as isEvalExpr2 } from "minato";
...
var SQLiteBuilder = class extends Builder {
  constructor(driver, tables) {
    ...
    this.evalOperators.$log = ([left, right]) => isNullable(right) ? `ln(${this.parseEval(left)})` : `ln(${this.parseEval(left)}) / ln(${this.parseEval(right)})`;
...
var _SQLiteDriver_decorators, _init, _a;
_SQLiteDriver_decorators = [Inject("logger", false)];
var SQLiteDriver = class extends (_a = Driver2) {
  static name = "sqlite";
  path;
  db;
  ...
  async start() {
    ...
    this.db.function("modulo", (left, right) => left % right);
  ...
  async stats() {
    const tables = [...this.tables];
    const pageCount = this._get(`PRAGMA page_count`);
    const pageSize = this._get(`PRAGMA page_size`);
    const stats = {
    ...
    tables.forEach((name) => {
      stats.tables[name] = this._get(`SELECT COUNT(*) as count FROM ${escapeId2(name)};`);
...
SQLiteDriver = __decorateElement(_init, 0, "SQLiteDriver", _SQLiteDriver_decorators, SQLiteDriver);
__runInitializers(_init, 1, SQLiteDriver);
((SQLiteDriver2) => {
...
var index_default = SQLiteDriver;
export {
  SQLiteDriver,

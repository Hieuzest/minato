...
function formatTime(time) {
  ...
  const month = import_cosmokit.Time.toDigits(time.getMonth() + 1);
  const date = import_cosmokit.Time.toDigits(time.getDate());
  const hour = import_cosmokit.Time.toDigits(time.getHours());
  const min = import_cosmokit.Time.toDigits(time.getMinutes());
  const sec = import_cosmokit.Time.toDigits(time.getSeconds());
  const ms = import_cosmokit.Time.toDigits(time.getMilliseconds(), 3);
  let timezone = import_cosmokit.Time.toDigits(time.getTimezoneOffset() / -60);
  if (!timezone.startsWith("-")) timezone = `+${timezone}`;
  return `${year}-${month}-${date} ${hour}:${min}:${sec}.${ms}${timezone}`;
}
__name(formatTime, "formatTime");
var PostgresBuilder = class extends import_sql_utils.Builder {
  constructor(driver, tables) {
    ...
    this.evalOperators = {
      ...
      $log: /* @__PURE__ */ __name(([left, right]) => (0, import_cosmokit.isNullable)(right) ? `ln(${this.parseEval(left, "double precision")})` : `(ln(${this.parseEval(left, "double precision")}) / ln(${this.parseEval(right, "double precision")}))`, "$log"),
      ...
      $or: /* @__PURE__ */ __name((args) => {
        ...
        if (import_minato.Field.boolean.includes(type.type)) return this.logicalOr(args.map((arg) => this.parseEval(arg, "boolean")));
        else return `(${args.map((arg) => this.parseEval(arg, "bigint")).join(" | ")})`;
      }, "$or"),
      $and: /* @__PURE__ */ __name((args) => {
        const type = import_minato.Type.fromTerm(this.state.expr, import_minato.Type.Boolean);
        if (import_minato.Field.boolean.includes(type.type)) return this.logicalAnd(args.map((arg) => this.parseEval(arg, "boolean")));
        else return `(${args.map((arg) => this.parseEval(arg, "bigint")).join(" & ")})`;
      }, "$and"),
      $not: /* @__PURE__ */ __name((arg) => {
        const type = import_minato.Type.fromTerm(this.state.expr, import_minato.Type.Boolean);
        if (import_minato.Field.boolean.includes(type.type)) return this.logicalNot(this.parseEval(arg, "boolean"));
        else return `(~(${this.parseEval(arg, "bigint")}))`;
      }, "$not"),
      $xor: /* @__PURE__ */ __name((args) => {
        const type = import_minato.Type.fromTerm(this.state.expr, import_minato.Type.Boolean);
        if (import_minato.Field.boolean.includes(type.type)) return args.map((arg) => this.parseEval(arg, "boolean")).reduce((prev, curr) => `(${prev} != ${curr})`);
        else return `(${args.map((arg) => this.parseEval(arg, "bigint")).join(" # ")})`;
      }, "$xor"),
      $get: /* @__PURE__ */ __name(([x, key]) => {
        const type = import_minato.Type.fromTerm(this.state.expr, import_minato.Type.Any);
      ...
      $number: /* @__PURE__ */ __name((arg) => {
        ...
        const res = import_minato.Field.date.includes(type.type) ? `extract(epoch from ${value})::bigint` : `${value}::double precision`;
    ...
    this.transformers["date"] = {
      ...
      dump: /* @__PURE__ */ __name((value) => (0, import_cosmokit.isNullable)(value) ? value : formatTime(value), "dump")
    };
  ...
  transformType(source) {
    const type = import_minato.Type.isType(source) ? source : import_minato.Type.fromTerm(source);
    if (import_minato.Field.string.includes(type.type) || typeof source === "string") return "text";
    else if (["integer", "unsigned", "bigint"].includes(type.type) || typeof source === "bigint") return "bigint";
    else if (import_minato.Field.number.includes(type.type) || typeof source === "number") return "double precision";
    else if (import_minato.Field.boolean.includes(type.type) || typeof source === "boolean") return "boolean";
  ...
  parseEval(expr, outtype = true) {
    ...
    return outtype ? this.encode(this.parseEvalExpr(expr), false, false, import_minato.Type.fromTerm(expr), typeof outtype === "string" ? outtype : void 0) : this.parseEvalExpr(expr);
  }
  ...
  createElementQuery(key, value) {
    if (this.isJsonQuery(key)) {
      const type = import_minato.Type.getInner(import_minato.Type.fromTerm(this.state.expr));
  ...
  createAggr(expr, aggr, nonaggr, eltype) {
    if (!this.state.group && !nonaggr) {
      ...
      return `(select ${aggr(`(${this.encode(this.escapeId("value"), false, true, void 0)})${eltype ? `::${eltype}` : ""}`)}
        from jsonb_array_elements(${value}) ${(0, import_minato.randomId)()})`;
    } else {
  ...
  parseSelection(sel, inline = false) {
    ...
    if (inline || !(0, import_minato.isAggrExpr)(expr)) {
      return `(SELECT ${output} FROM ${inner} ${(0, import_sql_utils.isBracketed)(inner) ? ref : ""}${where ? ` WHERE ${where}` : ""})`;
    } else {
      return [
        `(coalesce((SELECT ${this.groupArray(this.transform(output, import_minato.Type.getInner(import_minato.Type.fromTerm(expr)), "encode"))}`,
        `FROM ${inner} ${(0, import_sql_utils.isBracketed)(inner) ? ref : ""}), '[]'::jsonb))`
      ].join(" ");
    }
  }
  escapeId = escapeId;
  ...
  escapePrimitive(value, type) {
    if (value instanceof Date) {
      ...
      return `'\\x${import_cosmokit.Binary.toHex(value)}'::bytea`;
    } else if (import_cosmokit.Binary.isSource(value)) {
      return `'\\x${import_cosmokit.Binary.toHex(import_cosmokit.Binary.fromSource(value))}'::bytea`;
  ...
  toUpdateExpr(item, key, field, upsert) {
    ...
    for (const prop in item) {
      ...
      let escaped2;
      const v = (0, import_minato.isEvalExpr)(item[prop]) ? this.encode(this.parseEval(item[prop]), true, true, import_minato.Type.fromTerm(item[prop])) : (escaped2 = this.transform(this.escape(item[prop], type), type, "encode"), escaped2.endsWith("::jsonb") ? escaped2 : escaped2.startsWith(`'`) ? this.encode(`(${escaped2})::text`, true, true) : this.encode(escaped2, true, true));
...
function createIndex(keys) {
  return (0, import_cosmokit2.makeArray)(keys).map(escapeId).join(", ");
}
__name(createIndex, "createIndex");
var PostgresDriver = class extends import_minato2.Driver {
  ...
  _queryTasks = [];
  async start() {
    this.postgres = (0, import_postgres.default)({
...
parameters: %o` : ``), query, parameters.length ? parameters : "");
    ...
    this.define({
      ...
      load: /* @__PURE__ */ __name((str) => {
        if ((0, import_cosmokit2.isNullable)(str)) return str;
  ...
  async prepare(name) {
    ...
    for (const key in fields) {
      ...
      if (!column) {
        create.push(`${escapeId(key)} ${typedef} ${(0, import_cosmokit2.makeArray)(primary).includes(key) || !nullable ? "not null" : "null"}` + (!primary.includes(key) && !(0, import_cosmokit2.isNullable)(initial) ? " DEFAULT " + this.sql.escape(initial, fields[key]) : ""));
        ...
        update.push(`ALTER ${escapeId(key)} ${(0, import_cosmokit2.makeArray)(primary).includes(key) || !nullable ? "SET" : "DROP"} NOT NULL`);
        if (!(0, import_cosmokit2.isNullable)(initial)) update.push(`ALTER ${escapeId(key)} SET DEFAULT ${this.sql.escape(initial, fields[key])}`);
      }
    }
    if (!columns.length) {
    ...
    for (const key of unique) {
      ...
      const name2 = `unique:${table.name}:` + (0, import_cosmokit2.makeArray)(key).join("+");
      if (!oldIndex) {
  ...
  isDefUpdated(field, column, def) {
    ...
    } else if (import_minato2.Field.date.includes(field.deftype)) {
...
((PostgresDriver2) => {
  PostgresDriver2.Config = import_minato2.z.object({
    host: import_minato2.z.string().default("localhost"),
    port: import_minato2.z.natural().max(65535).default(5432),
